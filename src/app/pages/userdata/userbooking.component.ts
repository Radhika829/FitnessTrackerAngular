...
export class UserdataComponent {
  ...
  createChart() {
    ...
    const fieldName1 = 'weight';
    const fieldName2 = 'caleries';
    this.chart = new Chart('MyChart', {
      type: 'bar', //this denotes tha type of chart

      data: {
        // values on X-Axis
        labels: this.originalData.map((item) => item[fieldName]),
        datasets: [
          {
            label: 'Weight',
            data: this.originalData.map((item) => item[fieldName1]),
            backgroundColor: 'blue',
          },
          {
            label: 'Caleries',
            data: this.originalData.map((item) => item[fieldName2]),
            backgroundColor: 'red',
          },
        ],
      },
